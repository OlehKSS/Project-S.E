...
void MainWindow::showPointcloud(int idPointcloud){
    //Show selected pointcloud in the QVtk widget
...
void MainWindow::on_reconstructBtn_clicked()
{
    ...
    //Create new instance of the reconstruction_settings interface
...
void MainWindow::on_stitchBtn_clicked()
{
    ...
    //Create and show an instance of the ICP settings window
    ...
    //If the user hasen't closed the window
    while(Database->getICPSettingsStatus()==1){
        QCoreApplication::processEvents();
    }

    //If the user pressed the "Apply" button
    if(Database->getICPSettingsStatus()==2){
        ...
        //While the ICP is being performed
        while(Database->getIcpState()==0){
            QCoreApplication::processEvents();
        }

        //If ICP has finished correctly
...
void MainWindow::on_horizontalSlider_sliderMoved(int position)
{
    ...
    //Change point size according to slider value
...
{
    //If checked, connect to the turntable
    if(checked){
...
{
    //If checked, make the turntable to move
    if(checked){
        ttable_thread_class->terminate();
...
{
    //On value changed, save the port name for the turn table
    Database->setTtSerialPort(arg1);
}

void MainWindow::on_deleteBtn_clicked()
{
    //Delete the selected pointcloud from the list and from the vector of pointclouds
...
{
    //Delete all pointclouds from the vector
...
{
    //Clean visualizer and show the last pointclod in the vector
    resetVtk();
}

void MainWindow::on_actionNew_triggered()
{
    //Clear all the pointcloud variables
    Database->m_cloud.reset (new PointCloudT);
    resetVtk();
    Database->m_pointcloud_data.clear();
    updatePointcloudList();
...
void MainWindow::on_ttMove_released()
{
